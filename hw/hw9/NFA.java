...
public class NFA {
	...
	/**
	 * A char representing the empty character epsilon, the empty edge.
	 * Remainder: Character is the wrapped class of char.
	 */
	...
	private class State {
		...
		public Set<State> successors(char c) {
			...
			// Use DFS to recursively add all EPSILON edge states.
			Set<State> newSet = new HashSet<>();
			Stack<State> stack = new Stack<>();
			for (State state : _edges.get(EPSILON)) {
				stack.push(state);
			}
			// Be careful of empty-edge circle: don't let this loop run
			// forever in a empty-edge circle.
			while (!stack.isEmpty()) {
				State state = stack.pop();
				newSet.add(state);
				if (state._edges.containsKey(EPSILON)) {
					for (State newState : state._edges.get(EPSILON)) {
						if (!newSet.contains(newState)) {
							stack.push(newState);
						}
					}
				}
		 ...
		 */
		private final Map<Character, Set<State>> _edges =
				new HashMap<>();
	...
	public boolean matches(String query) {
		...
		for (int i = 0; i < query.length(); i++) {
			...
			Set<State> S_new = new HashSet<>();
			for (State q : S) {
				S_new.addAll(q.successors(c));
			}
			Set<State> temp = S;
			S = S_new;
			S_new = temp;
			S_new.clear();
			for (State q_new : S) {
				S_new.addAll(q_new.successors(EPSILON));
			}
			S.addAll(S_new);
		}
