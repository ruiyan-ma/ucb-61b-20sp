...
public class MatrixUtils {
	...
	static double get(double[][] e, int r, int c) {
		if (r >= 0 & c >= 0 & r < e.length & c < e[0].length) {
			return e[r][c];
		} else {
	...
	public static double[][] accumulateVertical(double[][] m) {
		...
		for (int i = 1; i < result.length; i++) {
			for (int j = 0; j < result[0].length; j++) {
				min = Math.min(Math.min(get(result, i - 1, j - 1), get(result, i - 1, j)), get(result, i - 1, j + 1));
				result[i][j] += min;
			}
		}
		return result;
	...
	static double[][] Transpose(double[][] m) {
		...
		double[][] mT = new double[colNum][rowNum];
	 ...
	 */
	static int findMin(double[] arr, int lo, int hi) {
		lo = Math.max(0, lo);
	...
	public static int[] findVerticalSeam(double[][] m) {
		...
		arr[m.length - 1] = findMin(m[m.length - 1], 0, m[0].length);
		for (int i = m.length - 2; i >= 0; i--) {
			arr[i] = findMin(m[i], arr[i + 1] - 1, arr[i + 1] + 1);
		}
		return arr;
	}
	...
	public static String matrixToString(double[][] m) {
		...
		for (double[] doubles : m) {
			for (int c = 0; c < width; c += 1) {
				sb.append(String.format("%9.0f ", doubles[c]));
			}
