...
class BoardWidget extends Pad {
    ...
    static final Color BLACK_COLOR = new Color(0.765f, 0.0f, 0.0117f),
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (Square sq : Square.ALL_SQUARES) {
            drawPiece(g, sq);
            if (_move != null && _destination.contains(sq)) {
                if (_board.turn() == WP) {
                    g.setColor(WHITE_COLOR);
                } else {
                    g.setColor(BLACK_COLOR);
                }
                g.fillOval(cx(sq) + 3 * PIECE_OFFSET, cy(sq) + 3 * PIECE_OFFSET,
                           PIECE_SIZE / 2, PIECE_SIZE / 2);
                g.setColor(PIECE_BOUNDARY_COLOR);
                g.setStroke(PIECE_BOUNDARY_STROKE);
                g.drawOval(cx(sq) + 3 * PIECE_OFFSET, cy(sq) + 3 * PIECE_OFFSET,
                           PIECE_SIZE / 2, PIECE_SIZE / 2);
            }
        }
    }
    ...
    private void mousePressed(Square s) {
        _move = s.toString();
        List<Move> moves = _board.legalMoves();
        for (Move move : moves) {
            if (move.getFrom() == Square.sq(_move)) {
                _destination.add(move.getTo());
            }
        }
        repaint();
    }
    ...
    private void mouseReleased(Square s) {
        _commands.offer(_move + "-" + s.toString());
        _move = null;
        _destination.clear();
        repaint();
    }
    ...
    /** Record the square waiting for move. */
    private Square _moveSQ = null;

    /** The list of possible destination. */
    private ArrayList<Square> _destination = new ArrayList<>();

    /** Queue on which to post move commands (from mouse clicks). */
    private ArrayBlockingQueue<String> _commands = null;
