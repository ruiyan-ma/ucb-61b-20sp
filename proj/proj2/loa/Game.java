...
class Game {
     ...
     *  program with non-zero code on receiving an erroneous move
     *  from a player. */
    ...
    /** Return a move or command from the standard input, after
     *  prompting if PROMPT. */
    ...
    /** Perform the move designated by LINE, if a valid move.
     *  Return true iff LINE has the syntax of a move. */
    private boolean processMove(String line) {
        ...
        if (move == null) {
            ...
            if (_PRINT & _board.turn() == manualPlayer()) {
                printBoard();
            }
            return true;
        }
    ...
    /** Return which side is auto player. */
    private Piece manualPlayer() {
        if (_white.isManual() & _black.isManual()) {
            return _board.turn();
        } else if (_white.isManual()) {
            return WP;
        } else if (_black.isManual()) {
            return BP;
        } else {
            return _board.turn();
        }
    ...
    /** Print an announcement of the winner.  Requires that the game
     *  has been won. */
    ...
    /** Print the BOARD. */
    void printBoard() {
        System.out.println(_board.print());
    }

    /** Set _board._movelimit equals to 2*STR. */
    void setLimit(String str) {
        char[] limitChar = str.toCharArray();
        int limit = 0;
        for (int i = 0; i < limitChar.length; i++) {
            limit += Math.pow(10, i) * (limitChar[limitChar.length - 1 - i] - ZERO);
        }
        _board.setMoveLimit(2 * limit);
    }
    ...
    /** True if actually playing (game started and not stopped or
     * finished). */
