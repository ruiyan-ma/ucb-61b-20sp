...
class PuzzleGenerator implements PuzzleSource {
     ...
     *  Unless ALLOW_FREE_ENDS, the first and last sequence numbers will
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        /* 必须要区分numbered和unnumbered两种情况:
         * numbered:
         *  (1) 有一个numbered succ并且是连续的=>唯一succ
         *  (2) 没有numbered succ，只有一个可连接的unnumbered succ=>唯一succ
         * unnumbered:
         *  只有一个可连接的unnumbered succ=>唯一succ*/
        Sq succsq = null;
        if (start.sequenceNum() != 0) {
            for (Place pl : start.successors()) {
                succsq = model.get(pl);
                if (succsq.sequenceNum() == start.sequenceNum() + 1) {
                    return succsq;
                }
            }
        }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        ...
        if (end.sequenceNum() != 0) {
            for (Place pl : end.predecessors()) {
                predsq = model.get(pl);
                if (predsq.sequenceNum() == end.sequenceNum() - 1) {
                    return predsq;
                }
            }
        }
