...
class Place {
    ...
    static class PlaceList extends ArrayList<Place> {
        ...
        /** PlaceList：存储place的list
         *  使用instace method: add(Place e)来添加place对象到PlaceList中
         *  使用pl(int x,int y)来创建新的place对象
        */
    ...
    static Place pl(int x, int y) {
        ...
        if (s >= _places.length) {
            ...
            _places = newPlaces;
            // 如果当前新建的place坐标大于_plac的维度，则重新构建_place
        }
        if (_places[x][y] == null) {
            _places[x][y] = new Place(x, y);
        }
        // 如果当前新建的place坐标之前未创建过，就加入_place
        return _places[x][y];
    }
    ...
    static PlaceList[][][] successorCells(int width, int height) {
        ...
        // FIXME
        /* M is just a 3 dimensions structured container,
         * every M[x][y][z] points to a PlaceList. */
        for (int x0 = 0; x0 < width; x0 += 1) {
            for (int y0 = 0; y0 < height; y0 += 1) {
                PlaceList[] places0 = M[x0][y0];
                /* Define a pointer of PlaceList, named places0.
                 * Assign the address stored in M[x0][y0] to places0.
                 * So places0[dir] is exactly M[x0][y0][dir]. */
                ...
                for (int x1 = 0; x1 < width; x1 += 1) {
                    for (int y1 = 0; y1 < height; y1 += 1) {
                        ...
                        if (dir != 0) {
                            /* when dir=0, (x0,y0) and (x1,y1) are the same point,
                             * (x1,y1) should not be added into any dir.
                             * (since there is no dir 0.) */
                            Place p = pl(x1, y1);
                            places0[dir].add(p);
                            places0[0].add(p);
                        }
